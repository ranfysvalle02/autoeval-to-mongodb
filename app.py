...
if MONGO_URI:    
    try:    
        ...
        evaluators_collection = db["evaluators"]  # Unified collection for all evaluators    
        idx_meta_collection = db["idx_meta"]  # Existing collection to store index metadata    
...
def load_evaluators_from_db():  
    try:  
        evaluators_cursor = evaluators_collection.find({})  
        for eval_doc in evaluators_cursor:  
            name = eval_doc.get('name')  
            if name in METRICS:  
                logger.info(f"Evaluator '{name}' already exists in METRICS, skipping...")  
                continue  
            ...
            if evaluator_type == 'LLMClassifier':  
                ...
                model_deployment_name = eval_doc.get('model_deployment_name', DEFAULT_DEPLOYMENT_NAME)  
                ...
                logger.info(f"Loaded LLMClassifier evaluator '{name}' from database.")  
                ...
                if function_subtype == 'regex_match':  
                    ...
                    if pattern:  
                        ...
                        METRICS[name] = evaluator  
                        logger.info(f"Added function-based evaluator '{name}' of subtype 'regex_match' to METRICS")  
                    else:  
                        logger.warning(f"Pattern not provided for evaluator '{name}'. Skipping.")  
                elif function_subtype == 'exact_match':  
                    # Use existing exact_match function  
                    METRICS[name] = exact_match  
                    logger.info(f"Added function-based evaluator '{name}' of subtype 'exact_match' to METRICS")  
                else:  
                    logger.warning(f"Unknown function_subtype '{function_subtype}' for evaluator '{name}'. Skipping.")  
            else:  
                logger.warning(f"Unknown evaluator type '{evaluator_type}' for evaluator '{name}'. Skipping.")  
    except Exception as e:  
        logger.exception("Error loading evaluators from database.")  
  
# Call the function to load evaluators from the database  
load_evaluators_from_db()     
        
# Load evaluators from the database and add them to METRICS    
try:    
    evaluators_cursor = evaluators_collection.find({})    
    for eval_doc in evaluators_cursor:    
        name = eval_doc.get('name')    
        if name in METRICS:    
            print(f"Evaluator '{name}' already exists in METRICS, skipping...", file=sys.stderr)    
            continue    
        evaluator_type = eval_doc.get('type', 'LLMClassifier')  # Default to LLMClassifier if not specified    
        ...
        if evaluator_type == 'LLMClassifier':    
            ...
            if function_subtype == 'regex_match':    
                ...
                if pattern:    
                    ...
                    evaluator = make_regex_evaluator(pattern)    
                    METRICS[name] = evaluator    
                    print(f"Added function-based evaluator '{name}' of subtype 'regex_match' to METRICS")    
                else:    
                    print(f"Pattern not provided for evaluator '{name}'. Skipping.", file=sys.stderr)    
            elif function_subtype == 'exact_match':    
                # Use existing exact_match function    
                METRICS[name] = exact_match    
                print(f"Added function-based evaluator '{name}' of subtype 'exact_match' to METRICS")    
            else:    
                print(f"Unknown function_subtype '{function_subtype}' for evaluator '{name}'. Skipping.", file=sys.stderr)    
        else:    
            print(f"Unknown evaluator type '{evaluator_type}' for evaluator '{name}'. Skipping.", file=sys.stderr)    
except Exception as e:    
    print(f"Error loading evaluators from database: {e}", file=sys.stderr)    
...
def store_evaluator_metadata(evaluator_config):  
    ...
    try:  
        ...
        existing_evaluator = evaluators_collection.find_one({"name": evaluator_config["name"]})  
        if existing_evaluator:  
            ...
            if config_changed:  
                ...
                evaluators_collection.update_one(  
                    {"_id": evaluator_id},  
            ...
            result = evaluators_collection.insert_one(evaluator_config)  
...
def run_rag_task(input_prompt: str, deployment_name: str, response_criteria: str, system_prompt_template: str, user_prompt_template: str,    
    ...
    context_docs = perform_vector_search(query_vector, db_name, collection_name, index_name, vector_field, selected_fields)    
    if not context_docs:    
...
def create_embedding_search_index(db_name, collection_name, index_name, embedding_field, num_dimensions):    
...
def process_documents(documents, collection, source_field, embedding_models):    
    bulk_ops = []    
        ...
        if text_to_embed and isinstance(text_to_embed, str):    
            ...
            for model_name, model_info in embedding_models.items():    
                ...
                if embedding:    
                    ...
                    print(f"Failed to generate embedding for model '{model_name}' and document with _id: {doc['_id']}", file=sys.stderr)    
            if update_fields:    
                bulk_ops.append(    
                    pymongo.UpdateOne(    
                        {'_id': doc['_id']},    
            ...
            if not source_field or ',' in source_field or ' ' in source_field.strip():    
                ...
                for model_name in selected_embedding_models:    
                    if model_name in EMBEDDING_MODELS:    
                        ...
                        safe_model_name = to_pascal_case(model_name)    
...
def manage_evaluators():  
    ...
    default_classifier = {  
        ...
        "use_cot": False,  
        "model_deployment_name": DEFAULT_DEPLOYMENT_NAME,  
        "temperature": 0.0  
    }  
    ...
    if request.method == 'POST':  
        ...
        if action == 'create_evaluator':  
            ...
            if evaluator_type == 'LLMClassifier':  
                ...
                model_deployment_name = request.form.get('model_deployment_name', '').strip() or DEFAULT_DEPLOYMENT_NAME  
            ...
            if not error_message:  
                ...
                evaluator_id = store_evaluator_metadata(evaluator_config)  
                if evaluator_id:  
            ...
            if evaluator_id:  
                ...
                result = evaluators_collection.delete_one({"_id": ObjectId(evaluator_id)})  
    ...
    evaluators = list(evaluators_collection.find({}))  
