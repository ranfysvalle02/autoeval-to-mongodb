...
import hashlib  # For hashing filters    
...
class MDBAutoEval:    
    ...
    def create_embedding_search_index(self, db_name, collection_name, index_name, embedding_field, num_dimensions):    
        """Create or update an Atlas Search index for the specified embedding field."""    
        try:    
            collection = self.mongo_client[db_name][collection_name]    
            ...
            self.logger.error(f"Error creating or updating search index {index_name} for {db_name}.{collection_name}: {e}")    
            return False    
